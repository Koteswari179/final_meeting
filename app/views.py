...
#         # ✅ Handle no selection
#         if not selected_ids:
#             return HttpResponse("<h3 style='color:red;text-align:center;'>⚠️ No meetings selected.</h3>")

#         meetings = Meeting.objects.filter(id__in=selected_ids)

#         # Create PDF response
#         response = HttpResponse(content_type='application/pdf')
#         response['Content-Disposition'] = 'attachment; filename="selected_meetings.pdf"'
...
#             spaceAfter=20,
#         )
#         elements.append(Paragraph("📅 Selected Meeting Details", title_style))
#         elements.append(Spacer(1, 0.2 * inch))

#         # Table data
#         data = [["S.No", "Date", "Event Time", "Subject", "Venue"]]
...
#                 str(m.event_time or ""),
#                 str(m.subject or ""),
#                 str(m.place_of_event or "")
#             ])

#         table = Table(data, colWidths=[40, 90, 90, 150, 130])
#         style = TableStyle([
...
#         for i in range(1, len(data)):
#             bg_color = colors.HexColor("#E9F3FF") if i % 2 == 0 else colors.whitesmoke
...
def download_meetings_pdf(request):
    if request.method == 'POST':
        ...
        # ✅ Take location from first meeting (if available)
        location_name = meetings.first().place_of_event if meetings and meetings.first().place_of_event else "Unknown Location"

        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = f'attachment; filename="meetings_{location_name}.pdf"'
        ...
        elements.append(Paragraph(f"📅 Selected Meeting Details – {location_name}", title_style))
        elements.append(Spacer(1, 0.2 * inch))
